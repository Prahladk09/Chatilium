    ...
    <div className="h-full w-20 lg:w-72 border-r border-base-300 flex flex-col transition-all duration-200">
     ...
     <div className='overflow-y-auto w-full py-3'>
        {filteredUsers.map((user, index) => (
            <button key={`${user.id}-${index}`} onClick={() => setSelectedUser(user)}
                ...
                ${selectedUser?._id === user._id ? 'bg-base-300 ring-1 ring-base-300' : ''}`}>
                    <div className='relative max-auto lg:mx-0'>
                        {user?.profilePic ? (
                            ...
                            <div className='size-12 flex items-center justify-center bg-base-200 text-base-content object-cover rounded-full'>
